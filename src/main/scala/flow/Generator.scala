...
object Generator {
  ...
  def apply[T, Out](source: Source[T, Future[Out]]): Generator[T, Out] =
    Generator(Future.successful(source))

  def empty[T, Out](implicit ec: ExecutionContext) = Generator.fromNotUsedSource[T](Source.empty[T])

  def fromIterator[T, Out](it: () ⇒ Iterator[T])(implicit ec: ExecutionContext) =
    Generator.fromNotUsedSource(Source.fromIterator(it))

  def fromFuture[T, Out](futureGen: Future[Generator[T, Out]])(implicit ec: ExecutionContext) =
    Generator(futureGen.flatMap(_.source))

  def failed[T, Out](e: Throwable) = Generator[T, Out](Future.failed[Source[T, Future[Out]]](e))
}

case class Generator[+T, +Out](source: Future[Source[T, Future[Out]]]) {
  ...
  private def use[U, Out2](source: Future[Source[U, Future[Out2]]]) = Generator(source)
}
