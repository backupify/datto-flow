...
case class FlowResult[+T, Ctx](value: Try[T], context: Ctx, metadata: Metadata = Metadata(), timings: TimingList = TimingList()) {
  // try interface
...
object FlowResultFutureImplicits {
  ...
  case class FutureWrapper[T, Ctx](future: Future[FlowResult[T, Ctx]]) {
    ...
    def flatMapResultValue[U](f: T ⇒ Try[U])(implicit ec: ExecutionContext): Future[FlowResult[U, Ctx]] =
      future.map { res ⇒
        try { res.flatMap(f) } catch { case e: Exception ⇒ FlowFailure[U, Ctx](e, res.context) }
      }
  }
}
...
private[flow] case class TimingList(created: Long = System.currentTimeMillis(), stages: Vector[TimingStage] = Vector[TimingStage]()) {
